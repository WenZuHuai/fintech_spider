...
class IndustryAndCommerceGeetestCrack(): 
    ...
    def get_trail_array(self, distance):
        ...
        for _ in range(last_move_distance):
            ...
            array_trail.append((x, y, t))
        result_array_trail = sorted(array_trail[:len(array_trail)], key=lambda x:x[2], reverse=True)
        result_array_trail += sorted(array_trail[len(array_trail):], key=lambda x:x[2])
        return result_array_trail
